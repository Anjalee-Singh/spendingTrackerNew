...
import static com.expense.model.PlatformResponseUtils.generateForError;
import static com.expense.model.PlatformResponseUtils.generateForTemplateError;
...
import com.expense.model.PlatformResponse;
...
public class ExpenseTrackerRestControllerAdvice {
	...
	private static final Logger LOGGER = LogManager
			.getLogger(ExpenseTrackerRestControllerAdvice.class);

	@ExceptionHandler(value = ExpenseException.class)
	public ResponseEntity<PlatformResponse> handleSrvException(
			ExpenseException e) {
		LOGGER.error("Handling SRVException - ", e);
		PlatformResponse resp = generateForError(e.getErrorCode().getCode(),
				e.getErrorMsg());
		return new ResponseEntity<>(resp, getStatus(e.getErrorCode()));
	}

	@ExceptionHandler(value = TemplateInputException.class)
	public ResponseEntity<PlatformResponse> handleTemplateException(
			TemplateInputException e) {
		LOGGER.error("Handling SRVException - ", e);
		PlatformResponse resp = generateForTemplateError(e.getTemplateName(),
				e.getMessage());
		return new ResponseEntity<>(resp, HttpStatus.FAILED_DEPENDENCY);
	}

	@ExceptionHandler(value = ServletException.class)
	public ResponseEntity<PlatformResponse> handleServletException(
			ServletException e) {
		LOGGER.error("Handling ServletException - ", e);
		PlatformResponse resp = generateForError(GENERAL_ERROR.getCode(),
				e.getMessage());
		return new ResponseEntity<>(resp, getStatus(GENERAL_ERROR));
	}

	@ExceptionHandler(value = HttpMessageNotReadableException.class)
	public ResponseEntity<PlatformResponse> handleHttpMessageNotReadableException(
			HttpMessageNotReadableException e) {
		LOGGER.error("Handling HttpMessageNotReadableException - ", e);
		PlatformResponse resp = generateForError(
				INVALID_INPUT_FORMAT.getCode(), "Invalid Input Error");
		return new ResponseEntity<>(resp, getStatus(INVALID_INPUT_FORMAT));
	}

	@ExceptionHandler(value = NoHandlerFoundException.class)
	public ResponseEntity<PlatformResponse> handleNoHandlerFoundException(
			NoHandlerFoundException e) {
		LOGGER.error("Handling NoHandlerFoundException - ", e);
		PlatformResponse resp = generateForError(
				GENERAL_RESOURCE_NOT_FOUND.getCode(), e.getMessage());
		return new ResponseEntity<>(resp, getStatus(GENERAL_RESOURCE_NOT_FOUND));
	}

	@ExceptionHandler(value = MethodArgumentTypeMismatchException.class)
	public ResponseEntity<PlatformResponse> handleMethodArgMismatchException(
			MethodArgumentTypeMismatchException e) {
		LOGGER.error("Handling handleMethodArgMismatchException - ", e);
		PlatformResponse resp = generateForError(
				INVALID_INPUT_REQUEST.getCode(), "Invalid Input Error");
